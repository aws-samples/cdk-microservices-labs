...
class ServerlessCicdStack(core.Stack):
    ...
    def __init__(self, scope: core.Construct, id: str, lambda_code: _lambda.CfnParametersCode, custom_resource: _lambda.CfnParametersCode, **kwargs) -> None:
        ...
        code = _commit.Repository(self, 'VisitServerlessCode',
        	   repository_name='spring-petclinic-visits-serverless'
        	)

        lambda_project = _build.PipelineProject(self, 'VisitLambdaBuild',
	      build_spec=_build.BuildSpec.from_object({
            ...
            'post_build': {
               'commands': [
                  'mkdir deploy',
                  'cp target/spring-petclinic-visits-serverless-2.0.7.RELEASE.jar deploy/',
                  'cd deploy && jar xvf spring-petclinic-visits-serverless-2.0.7.RELEASE.jar',
                  'rm spring-petclinic-visits-serverless-2.0.7.RELEASE.jar',
               ]
            }
	        },
	        'artifacts': {
        ...
        cdk_project = _build.PipelineProject(self, 'VisitCdkBuild',
	    	build_spec=_build.BuildSpec.from_object({
               'version': 0.2,
               'phases': {
                  'install': {
               ...
               'artifacts': {
                   'secondary-artifacts': {
                      'CdkBuildOutput': {
                        'base-directory': 'dist',
                        'files': ['visit-lambda-stack.template.json']
                      },
                      'CustomRecoureOutput': {
                        'base-directory': 'custom-resource-code',
                        'discard-paths': 'yes',
                        'files': ['index.py', 'visit.json', 'cfnresponse.py']
                      }
                   }
                }
	    	}),
                ...
                stages=[
                    {
                        ...
                        'actions': [_action.CloudFormationCreateUpdateStackAction(
                            ...
                            template_path=cdk_build_output.at_path('visit-lambda-stack.template.json'),
                            stack_name='visit-lambda-stack',
